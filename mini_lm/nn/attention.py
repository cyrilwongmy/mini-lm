...
from . import Softmax, RotaryPositionEmbedding
...
class MultiHeadAttentionWithRope(Module):
    ...
    def forward(self, query, key, value, token_positions:Optional[Tensor]=None):
        ...
        rope = RotaryPositionEmbedding(theta=self.theta, d_k=self.k_dim, max_seq_len=self.max_seq_len)

        # Expand token_positions to match the combined batch dimension
        token_positions_expanded = token_positions.unsqueeze(1).expand(batch_size, self.num_heads, seq_len)
        token_positions_expanded = token_positions_expanded.reshape(batch_size * self.num_heads, seq_len)
